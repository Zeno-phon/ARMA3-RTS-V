...
Zen_RTS_F_East_Repairer = {
    if (isServer) then {
        ...
        if (alive (_buildingObjData select 2)) then {
            ZEN_RTS_STRATEGIC_ASSET_SPAWN_MESSAGE() \
            _group = [(_buildingObjData select 2), "rhs_msv_rifleman"] call Zen_SpawnGroup;
        ...
        ZEN_FMW_MP_REServerOnly("Zen_RTS_F_East_Repairer", _this, call)
    };
};
...
Zen_RTS_F_East_Recycler = {
    if (isServer) then {
        ...
        ZEN_FMW_MP_REServerOnly("Zen_RTS_F_East_Recycler", _this, call)
    };
};
